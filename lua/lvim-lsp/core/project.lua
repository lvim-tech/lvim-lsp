        ...
        if is_array then
            ...
            for _, k in ipairs(keys) do
                local key = type(k) == "string"
                    and (k:match("^[%a_][%w_]*$") and k or string.format("[%q]", k))
...
function M.save(root_dir, data)
    ...
    if ok then _config_cache[root_dir] = data end
    return ok
end
...
function M.load_server(root_dir, server_name)
    ...
    if _server_cache[key] ~= nil then return _server_cache[key] end
...
function M.save_server(root_dir, server_name, data)
    ...
    if ok then _server_cache[root_dir .. "/" .. server_name] = merged end
    return ok
end
...
function M.load_ft(root_dir, ft)
    ...
    if _ft_cache[key] ~= nil then return _ft_cache[key] end
...
function M.save_ft(root_dir, ft, data)
    ...
    if ok then _ft_cache[root_dir .. "/" .. ft] = data end
    return ok
end
...
function M.load_efm_tool(root_dir, tool_name)
    ...
    if _efm_tool_cache[key] ~= nil then return _efm_tool_cache[key] end
...
function M.save_efm_tool(root_dir, tool_name, data)
    ...
    local merged   = deep_merge(existing, data)
    local ok = write_file(path, merged)
    if ok then _efm_tool_cache[root_dir .. "/" .. tool_name] = merged end
    return ok
end

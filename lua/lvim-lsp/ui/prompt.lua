...
function M.add_pending(ft, server_name, mod)
	if snoozed[ft] and snoozed[ft][server_name] and vim.uv.now() < snoozed[ft][server_name] then
		return
	end
	...
	for _, ft in ipairs(fts) do
		...
		ui_mod.multiselect({
			...
			callback = function(confirmed, selected)
				...
				require("lvim-lsp.ui.installer").ensure_mason_tools(to_install, function()
